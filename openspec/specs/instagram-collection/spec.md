# instagram-collection Specification

## Purpose

通过 Playwright 驱动真实浏览器采集 Instagram 内容（单帖与 Profile 分页），以"合法 Web 客户端"
的身份加载页面并拦截浏览器自身的网络响应来提取数据，取代 instaloader 成为主采集路径。
采集结果必须完整填充现有响应模型，且所有对外接口（`/preview`、`/profile/preview`、
`/profile/download`、`/download`、`/media-proxy`）的契约逐字段保持不变，前端零改动；
下载继续使用采集层产出的 CDN 直链经现有 `httpx` 直拉。
